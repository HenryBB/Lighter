...
public class Ray implements Comparable<Ray>{
	...
	private int calculateQuadrant() {
		...
		else if ((tip.getY()>origin.getY())&&(tip.getX()<=origin.getX())) {
			return 2;
		}
		else if ((tip.getY()<=origin.getY())&&(tip.getX()>origin.getX())) {
			return 3;
	...
	public int compareTo(Ray ray) {
		...
		if (quadrant!=rQuadrant) {
			...
			else {return -1;} //otherwise the first ray is less than the second ray
		}
		else { //if both rays are in the same quadrant, the ray with the larger slope has a bigger angle
			if (slope==r.getSlope()) {return 0;} //rays are equal if slopes and quadrants are the same
			if (slope>r.getSlope()) {return 1;} //the slope of the first ray is greater, thus has a larger angle
			else {return -1;} //otherwise the first ray has a smaller slope and thus has a smaller angle
		}
	}
